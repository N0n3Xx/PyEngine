	...
	def generate(self):
		for z in range(CHUNK_SIZE):
			for x in range(CHUNK_SIZE):
				...
				height = self.terrain.generate_height(global_x, -global_z)
				self.generate_cell(Vec3(x, 0, z), z * CHUNK_SIZE + x, height)

	def mesh_chunk(self):
		vertices, indices = self.mesher.triangulate(self.cells)
		self.mesh = Mesh.create(self.terrain.device, vertices, indices)

	def generate_cell(self, cell_coord, index, height):
		local_pos = Vec3(
